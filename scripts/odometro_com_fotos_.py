...
measured_shortest_dist_y = np.mean([25, 25, 25])  # 3 measurements were made, where distance is express in mm
measured_longest_dist_x = np.mean([50, 50, 50])  # 3 measurements were made, where distance is expressed in mm
...
svd_traj.compute_total_trajectory_path(data_root, n_images=589)
...
plt.xlim(
    measured_longest_dist_x/2 - measured_longest_dist_x/2,
    measured_longest_dist_x/2 + measured_longest_dist_x/2)
plt.ylim(
    measured_longest_dist_x/2 - measured_longest_dist_x/2 - 5,
    measured_shortest_dist_y/2 + measured_longest_dist_x/2 - 5)
...
plt.tight_layout()
plt.savefig("frames/Figure3d.svg", format=None, metadata=None, bbox_inches="tight")
